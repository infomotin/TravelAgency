...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3">New Transaction</h1>
    ...
    <div class="card-body">
        <form action="{{ route('transactions.store') }}" method="post" class="row g-3">
            ...
            <div class="col-md-3">
                ...
                <input type="date" name="date" value="{{ old('date', date('Y-m-d')) }}" class="form-control form-control-sm @error('date') is-invalid @enderror">
                @error('date')
            ...
            <div class="col-12">
                ...
                <textarea name="description" rows="2" class="form-control form-control-sm @error('description') is-invalid @enderror">{{ old('description') }}</textarea>
                @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-12 text-end">
                <button class="btn btn-primary btn-sm">Save Transaction</button>
            </div>
        </form>
    </div>
</div>
